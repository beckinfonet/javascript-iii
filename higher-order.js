...
let evenNumbers = mixedNumbers.filter(function(item){
    return (item % 2 === 0);
})
...
let postTaxPrices = prices.map(function(item){
    return item + (item * 0.07);
})
...
let totalPopulation = populations.reduce(function(acc, item){
    return acc + item;
})
...
let myStrongest = monstersInYourPocket.filter(function(item){
    return item.CP > 200;
})
...
let bobsTotal = purchases.reduce(function(acc, val){
    if(val['owner'] === 'Bob'){
